...
model.columns = require('./links.js').columns;
...
var xmbVue = window.xmbVue = new Vue({
  el: "#xmb",
  data: model,
  methods: {
...
jquery('body').on("mousewheel", _.throttle(scrollHandler, 10));

function scrollHandler(e) {
  var event = e.originalEvent;
  if (event.deltaX) {
    xmbVue.handleKey('x', Math.sign(event.deltaX));
  }
  if (event.deltaY) {
    xmbVue.handleKey('y', Math.sign(event.deltaY));
  }
}
